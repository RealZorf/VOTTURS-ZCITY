...
module[2] = function(owner, org, timeValue)
	...
	local cigaretteRegenMul = math.Clamp(org.cigaretteStaminaRegenMul or 1, 0, 1)
	local ragdoll = owner.FakeRagdoll
	local climbing = IsValid(ragdoll) and (
		(IsValid(ragdoll.ConsLH) and ragdoll.ConsLH.ZCClimbGrip)
		or (IsValid(ragdoll.ConsRH) and ragdoll.ConsRH.ZCClimbGrip)
	)
	local staminaRegenMultiplier = climbing and 0.35 or 1

	if climbing and org.o2[1] >= 10 then
		local climbStaminaMultiplier = math.Clamp(tonumber(owner.ClimbStaminaMul) or 1, 0.1, 2)
		stamina.sub = math.max(stamina.sub, 0.75) * climbStaminaMultiplier
	end
