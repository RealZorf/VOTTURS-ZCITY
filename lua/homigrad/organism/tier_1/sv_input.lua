...
local THROAT_CUT_MIN_DAMAGE = 6
local THROAT_CUT_NECK_DIST_SQR = 15 * 15
...
hook.Add("EntityTakeDamage", "homigrad-damage", function(ent, dmgInfo)
	...
	--if ent:IsRagdoll() then
		...
		if dmgInfo:IsDamageType(DMG_BULLET) and RagdollDamageBoneMul[hitgroup] then
			...
			--I don’t even know, maybe I should remove this again? ^
			--It’s been wrong for us for so long that it’s probably
	...
	if dmgInfo:IsDamageType(DMG_BULLET + DMG_BUCKSHOT + DMG_BLAST + DMG_SLASH + DMG_CLUB + DMG_GENERIC) then
		...
		if ply then
			...
			if ply.AddForceRag[bone][2] and ply.AddForceRag[bone][2]:Length() > 4500 then --In my opinion, some big values, no?
...
--[[hook.Add("HomigradDamage", "painsounds",function(ply, dmgInfo, hitgroup, ent) -- Example HomigradDamage usage
...
local function velocityDamage(ent, data)
	...
	if not org.superfighter then
		...
		if hitgroup == HITGROUP_STOMACH and physAng:Forward():Dot(data.HitNormal) > 0.6 then hg.organism.input_list.spine1(org, bone, dmg * (math.random(3) > 1 and 1 or 0) * 3, dmgInfo) end --| AND WHY IS OUR BACK BROKEN FROM FALLING ON THE CHEST OR Stomach...
